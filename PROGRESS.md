...
- **Build 18 alındı, kuruldu → HİÇBİR ŞEY DEĞİŞMEDİ** (avatarlar hâlâ yok, donma sürüyor). Yani ticker fix + svg yükseltme + Live Activity kapatma: üçü de değil.

**🛑 TEORİ ÇÖKTÜ — bir sonraki oturum buradan başlasın:**
- **Build 12 (ÇALIŞAN) de `iPhoneOS26.0.sdk` ile derlenmiş** (build log'undan doğrulandı). Yani "iOS 26 SDK toolchain'i bozdu" teorisi **geçersiz**; MMKV/SVG/Live Activity zincirinin tamamı bu yanlış dayanağa oturuyordu. `react-native-svg` 15.15.5 yükseltmesi **geri alındı** (`c72c6e4`) — Expo SDK 54'ün matrisi 15.12.1; ayrıca `expo` 54.0.35 → **54.0.36**.
- **Kod farkı ÇOK DAR**: build 12 (`d73128d`) → build 14 (`513d415`) arasında mobil tarafta sadece **iki commit** var: `f9b2ed2` (avatar görünürlük fix'i: Home hero + Friends/Rooms satırlarında `AvatarArt`) ve `513d415` (MMKV pin config plugin'i → Podfile'a `pod 'MMKV'/'MMKVCore' 2.4.0` ekliyor). Gerisi dokümantasyon. **Avatarların iOS'ta kaybolması bu ikisinden birinden gelmek zorunda.**
- **Donma bundan ESKİ**: kullanıcı "önceki build'de de vardı" dedi → build 12'de de donuyordu. Yani donma ile avatar kaybı **ayrı iki sorun** olabilir ve donmanın sebebi ≤ build 12 (Faz 31/32/33 veya öncesi).
- **Elenenler**: backend/taşıma (uçlar 200 + gerçek veri), EAS env (DSN gömülü), OTA (`expo-updates` YOK → cihazda gerçekten yeni kod var), iOS'ta çift SVG paketi (tek pod/tek lockfile girdisi), yeni mimari farkı (Android da `newArchEnabled=true`), Sentry'deki RNSVG fatal'leri (kullanıcı doğruladı: hepsi Android/dev = Claude'un emülatörü).
- **Android'de HİÇBİR SORUN YOK** (emülatör reprodüksiyonu ile doğrulandı) → sorun iOS'a özel ama sebebi bilinmiyor.

**⏭️ SONRAKİ OTURUMUN İLK İŞLERİ (sırayla):**
1. **Kullanıcıdan 3 ekran görüntüsü** (bedava, en yüksek bilgi): (a) zamanlayıcı başlatıldıktan ~10 sn sonra, (b) Profil → avatar bölümü, (c) "loading'de takılı" bir ekran. Sorular: gerçekten `ActivityIndicator` mı yoksa boş mu; **SVG olan ilerleme arkı + kuyruklu yıldız başı çiziliyor mu** (kıvılcımlar/tik çerçevesi View olduğu için ayrım tanı verir); UI hangi durumda kalmış.
2. **iOS dev client** (kör döngüyü bitiren asıl adım): `eas device:create` (kullanıcı telefondan link açıp profil kurar, tek sefer) → `eas build -p ios --profile development` (profil `eas.json`'da hazır) → kullanıcı kurar, Claude Metro'yu başlatır (aynı WiFi) → **canlı konsol + hata + anında reload**. 20 dakikalık kör build döngüleri yerine saniyelik iterasyon.
3. Elde veri olunca: `f9b2ed2`'yi geçici geri alıp (avatar render yolu) iOS'ta avatarların dönüp dönmediğine bakmak — build 12↔14 farkını ikiye böler.
4. Sentry'de `os:iOS` + **`js-thread-stall`** olayı var mı (build 17+ watchdog'u). **Yoksa** JS thread tıkalı DEĞİL demektir → sorun "render/commit ekrana yansımıyor" tarafındadır, teşhis oraya kayar.
- **Not**: Sentry artık `__DEV__`'de susturuldu (`1309b42`), emülatör gürültüsü bir daha gerçek cihaz hatası gibi görünmeyecek.
...
- **🍎 ASC v1.0 SAYFASI TAMAMLANDI** (aynı oturum, Claude tarayıcıyı kullandı): **Türkçe lokalizasyon eklendi** — Version 1.0 TR (promo/description/keywords) + App Information TR (**ad: "StudySquad: Arkadaşınla Çalış"** 29/30, altyazı "Arkadaşlarınla odaklan"). Birincil dil **English (U.S.) kaldı**. Kullanıcı **16 ekran görüntüsünü** sürükledi (EN 8 + TR 8, ikisi de doğru sırada), **Sign-In password + telefon** doldurdu. Build 10 bağlı. **Yayın şekli: kullanıcı "Automatically release" seçti** (eski plan manuel'di).
...
- **FIX (backend, authoritative + geriye dönük tüm istemcileri korur)**: `stopTimer` artık `durationMinutes`'ı `state.duration` ile cap'liyor (`Math.min(rawElapsed, duration*60000)`); `getTimerStatus` `elapsedMs`'i de cap'liyor. Tamamlanma (`wasCompleted`) hâlâ **ham** elapsed'e göre (0.9×). Geri-sayım seansı asla set edilen süreden fazla kaydetmez. **Not:** seansın *intended* süresi DB'de saklanmıyor (sadece Redis state'teydi) → eski şişmiş satırlar precise cap'lenemez; temizlik = eşik üstü test seanslarını silmek (kullanıcı onayı bekliyor).
- **FEATURE (kullanıcı istedi: "ayarladığım süre kadar çalışınca bildirim")**: klasik modda set edilen süreye ulaşınca **yerel bildirim** ("⏰ Süren doldu!"). Pomodoro'nun `scheduleRoundEnd` deseninin aynısı (`scheduleClassicEnd` TimerScreen'de): başlangıçta planlanır → **kilitli telefonda bile OS teslim eder**, duraklamada iptal, devamda kalan süreyle yeniden planlanır, durma/doğal-tamamlanmada iptal. Klasik doğal tamamlanmada titreşim de eklendi. i18n 10 dilde `timer.sessionDoneTitle/Body`.
- **✅ Backend Fly deploy YAPILDI** (24 Tem, cap fix canlı, `/health` 200).
- **✅ Eski şişmiş test verisi TEMİZLENDİ** (24 Tem, `duration_minutes > 180` sildi — `scratchpad/cleanup-inflated-sessions.sql`): **4 seans silindi** (Kpss2026 3 seans ~30sa toplam + focushero 1 seans ~17sa). Kalan >180 = 0. XP/coin geri hesaplanmadı (test hesabı); `room_member_minutes` kümülatifi dokunulmadı (bugünkü dakika canlı sessions'tan → düzeldi).
- **✅ iOS production build 11 BAŞARILI + TestFlight'a SUBMIT edildi** (24 Tem): build `6f34f071` (buildNumber **11**, v1.0.0), auto-submit zincirlendi (submission `f77843ae`, ASC key EAS'te). Faz 31 (oda ekranı) + Faz 32 (timer cap + "süren doldu" bildirimi) birlikte. Apple processing sonrası TestFlight'ta. **⏳ Cihaz testleri bekliyor**: (1) timer cap — kısa timer→kilitle→15+dk bekle→aç→set süre kadar kaydedilmeli; (2) "⏰ Süren doldu!" bildirimi kilit ekranında; (3) oda ekranı varlık merdiveni + son görülme + üye konu alt-sayfası.

### Faz 31 — 🏠 Oda ekranı "canlı varlık" yenilemesi + üye konu kırılımı (24 Temmuz)
...
- **Gün içi devam 4 — BUILD 9 + kalıntı eas.json temizliği**: Kullanıcı "autoIncrement çalışmıyor" sandı — sebep repo kökündeki KULLANILMAYAN `eas.json` (remote appVersionSource yazıyordu; gerçek config `mobile/eas.json` = local + autoIncrement). Kök dosya silindi (`f576007`). Mekanizma açıklandı: numara production build BAŞLATILINCA artar (6→7→8 hep böyle olmuştu). **Yeni build'ler atıldı** (tepsi fix `dc1a41a` + pet vitrini `225cb7e` dahil): **iOS production `bd47dcd9` (buildNumber 9, auto-submit zincirlendi, submission `1c759dd5`)**. Android preview `d4a8bac6` de başlatılmıştı ama kullanıcı "sadece iOS" dedi → **iptal edildi** (Faz 25 fix'leri Android build'ine ileride girecek). app.json bump commit: `0b34033`. Bitince: TestFlight build 9 cihaz testleri (pomodoro senaryoları + pet mağazası vitrini + tepsi temizliği).
