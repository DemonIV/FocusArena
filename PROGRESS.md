...
- **⏳ SONRAKİ ADIM**: (1) **backend Fly deploy** — `flyctl deploy --depot=false` (değişiklik additive/geri-uyumlu, eski build'ler etkilenmez, migration zaten uygulandı); (2) yeni mobil build → cihazda test: avatar ızgarası doğru gradyanlarla mı çiziliyor, kilitli/açık ayrımı, takma sonrası ana sayfa + arkadaş/oda/leaderboard'da görünme. **Not**: iOS `buildNumber` autoIncrement ile **12**'ye çıktı (build 12 = avatarlar, ama görünürlük fix'i **yok** — fix bir sonraki build'e girer).
